...
def training_loop(model, dataloader, optimizer, num_epochs, num_timesteps, device=device):
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch+1) % 6 == 0: 
                ...
                generated = generate_image_inpainting(model,batch[:8], 8, 3, 32, mask, 5)
                show_images(generated, "Final result")
                plt.tight_layout()
                plt.savefig(f'results_epoch_{epoch+1}_ddpm_inpainting.png')  
                plt.close()  
                generated = generate_image(model, 16, 3, 32)
                show_images(generated, "Final result")
                plt.tight_layout()
                plt.savefig(f'results_epoch_{epoch+1}_ddpm_cifar_10.png')  
                plt.close() 
                print("sampled")
...
dataloader = torch.utils.data.DataLoader(dataset=dataset, batch_size=256, shuffle=True,num_workers=2)

for b in dataloader:
    batch = b[0]
    print(batch[:64].shape)
    break
...
sched_mod = torch.optim.lr_scheduler.StepLR(optimizer=optimizer, step_size=1700, gamma=0.2)
