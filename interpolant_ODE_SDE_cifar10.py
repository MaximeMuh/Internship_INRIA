...
Original file is located at
    https://colab.research.google.com/drive/1RlOJrmzqBbFXMLwEf3Q5WmrZHCTumae0
"""
...
transform = transforms.Compose([
    ...
    transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010))  # Normalisation basée sur les statistiques CIFAR-10
])
...
print(images.shape)  # Devrait afficher torch.Size([64, 3, 32, 32])
print(labels.shape)  # Devrait afficher torch.Size([64])

# Afficher quelques images pour vérifier que tout fonctionne correctement
import matplotlib.pyplot as plt
import numpy as np

def imshow(img):
    img = img * 0.2 + 0.5  # dé-normaliser
    npimg = img.numpy()
    plt.imshow(np.transpose(npimg, (1, 2, 0)))
    plt.show()

# Obtenir quelques lots de données d'entraînement
data_iter = iter(train_loader)
images, labels = next(data_iter)
...
opt_b = torch.optim.Adam(b.parameters(), lr=2e-4)
opt_eta = torch.optim.Adam(eta.parameters(), lr=2e-4)
epochs = 5
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
...
# Charger l'état des modèles et des optimisateurs
b.load_state_dict(checkpoint['model_b_state_dict'])
eta.load_state_dict(checkpoint['model_eta_state_dict'])
opt_b.load_state_dict(checkpoint['optimizer_b_state_dict'])
opt_eta.load_state_dict(checkpoint['optimizer_eta_state_dict'])
...
b.eval()
eta.eval()
sde_flow = SDEIntegrator(b=b, s=s, eps=torch.tensor(0.01), n_save=4, start_end=(0.0001, 0.999), n_step=10**2).to(device)
pflow = PFlowIntegrator(b=b,  method='dopri5', interpolant=interpolant, n_step=3)
bs = 64
with torch.no_grad():
    x0_tests  = torch.randn(bs,3, 32, 32).to(device)
    print(x0_tests.shape)
...
b2 = Unet2(dim=64, channels=1).to(device)
eta2 = Unet2(dim=64,channels=1).to(device)
opt_b2     = torch.optim.Adam(  b2.parameters(), lr=base_lr)
opt_eta2   = torch.optim.Adam(eta2.parameters(), lr=base_lr)
sched_b2   = torch.optim.lr_scheduler.StepLR(optimizer=  opt_b2, step_size=1500, gamma=0.4)
sched_eta2 = torch.optim.lr_scheduler.StepLR(optimizer=opt_eta2, step_size=1500, gamma=0.4)
...
for epoch in range(epochs):
    ...
    for x1, label in train_loader:
        ...
        loss_b = make_loss(loss_per_sample_b_one_sided, b2, x0, x1, ts, interpolant)
        loss_eta = make_loss(loss_per_sample_one_sided_eta, eta2, x0, x1, ts, interpolant)
    ...
    torch.save({
        'epoch': epoch + 1,
        'model_b_state_dict': b2.state_dict(),
        'model_eta_state_dict': eta2.state_dict(),
        'optimizer_b_state_dict': opt_b2.state_dict(),
        'optimizer_eta_state_dict': opt_eta2.state_dict()
    }, f'mnist_checkpoint_epoch_{epoch+1}.pth')

def grab(var):
    return var.detach().cpu().numpy()
s2 = SFromEta(eta2, interpolant.a)

sde_flow = SDEIntegrator(b=b2, s=s2, eps=torch.tensor(0.01), n_save=4, start_end=(0.0001, 0.999), n_step=10**2).to(device)
pflow = PFlowIntegrator(b=b2,  method='dopri5', interpolant=interpolant, n_step=3)
bs = 64
with torch.no_grad():
