    ...
    with torch.no_grad():
        ...
        for t in timesteps:
            for u in range(iter):
                ...
                residual = ddpm.forward(xt, time_tensor).to(device)
...
mask = transform(image_pil).to(device)
mask = mask[0]

#%%
num_timesteps=1000
model = Unet(dim=64).to(device)
model_save_path = 'results_mazes_inpainting/model_3/model_epoch_31.pth'
model = torch.load(model_save_path)
model.eval()
print("load")
ddpm = DDPM(model, num_timesteps, beta_start=0.0001, beta_end=0.02, device=device)
...
batch = next(train_iter)[0].to(device)
...
plt.savefig(f'results_mazes_inpainting/mask_random/4_result_mask.png')  
plt.close()  

generated = generate_image_inpainting(ddpm,batch[:4], 4, 1, 64, mask, 10)

plt.figure(figsize=(10, 5))  

plt.subplot(1, 2, 1)
initials = torchvision.utils.make_grid(batch[:4].cpu(), nrow=2, padding=2)
imshow(initials)
plt.title('initial (64x64)')
plt.axis('off')

plt.subplot(1, 2, 2)
generated_grid = torchvision.utils.make_grid(torch.tensor(generated), nrow=2, padding=2)
imshow(generated_grid)
plt.title('Results (64x64)')
plt.axis('off')

plt.tight_layout()
plt.savefig(f'results_mazes_inpainting/mask_random/4_result_10.png')
plt.show()