...
def training_loop(model, dataloader, optimizer, num_epochs, num_timesteps, device=device):
    ...
    plt.axis('off')
    plt.tight_layout()
    plt.savefig(f'results_mazes_inpainting/model_3/mask.png')
    plt.close()
    global_step = 0
    losses = []
    ...
    for epoch in range(num_epochs):
        ...
        for step, batch in enumerate(dataloader):
            ...
            #print(step)
        #progress_bar.close()
        print(f'epoch{epoch} finished, epoch_loss = {epoch_loss}, loss = {loss.detach().item()}')
        if (epoch+1) % 2 == 0: 
                generated = generate_image_inpainting(model,batch[:8], 8, 1, 64, mask, 3)
                imshow(torchvision.utils.make_grid(torch.tensor(generated)))
                plt.tight_layout()
                plt.savefig(f'results_mazes_inpainting/mask_4/results_epoch_{epoch+1}_mazes_inpainting_2.png')  
                plt.close()  
                generated = generate_image(model, 8, 1, 64)
                imshow(torchvision.utils.make_grid(torch.tensor(generated)))
                plt.tight_layout()
                plt.savefig(f'results_mazes_inpainting/mask_4/results_epoch_{epoch+1}_ddpm_mazes_2.png')  
                plt.close() 
                print("sampled")
        if (epoch ) % 5 == 0:
                model_save_path = f'results_mazes_inpainting/model_3/model_epoch_{epoch+1}.pth'
...
sched_mod = torch.optim.lr_scheduler.StepLR(optimizer=optimizer, step_size=2500, gamma=0.2)
training_loop(model, train_loader, optimizer, num_epochs, num_timesteps, device=device)
