...
path = "~/repos/102flowers"
data_path = "~/repos/102flowers"
transform = transforms.Compose([transforms.Resize((128, 128)), transforms.ToTensor(),transforms.Normalize(mean=[0.43560296, 0.37773207, 0.28832594], std=[0.25887805, 0.20485985, 0.21232468])])
...
mean = [0.43560296, 0.37773207, 0.28832594]
std = [0.25887805, 0.20485985, 0.21232468]
...
for epoch in range(epochs):
    ...
    if (epoch + 1) % 30 == 0 and epoch_loss < best_loss:
        ...
        }, f'checkpoint_epoch_{epoch+1}_flowers_2.pth')
    ...
    if epoch == 2 or (epoch + 1) % 5 == 0:  
        with torch.no_grad():
            ...
            b.eval()
            eta.eval()
            sde_flow = SDEIntegrator(b=b, s=s, eps=torch.tensor(0.1), n_save=4, start_end=(0.001, 0.999), n_step=10**2).to(device)
            bs = 4
            x0_tests = torch.randn(bs, 3, 128, 128).to(device)
            xfs_sde = sde_flow.rollout_forward(x0_tests, 'heun')
            xf_sde = grab(xfs_sde[-1].squeeze())
            xf_sde = xf_sde.reshape(4, 3, 128, 128)
            selected_images = xf_sde[:4,:,:,:]

            imshow(torchvision.utils.make_grid(torch.tensor(selected_images)))

            plt.tight_layout()
            plt.savefig(f'results_epoch_{epoch+1}_flowers_2.png')  
            plt.close()  
print("fin train")
